...
"""
This code does the actual aircraft wikipedia page scraping. It reads in the 
list of aircraft links that was scraped in the 01 script, and then scrapes each
individual aircraft's wikipedia page, saving that data locally. It specifically
scrapes and parses the information in the infobox and the specifications section
of the wikipedia entry, as those contain the information that will populate
...
# Read in the links generated by the 01 script
df_aircraft = pd.read_csv('Script_Data/01_1_wiki_aircraft_links.csv')

# Checking if the titles were mismatched, roughly 271 were mismatched, though
# many of those were differences in special characters.
df_aircraft['title_mismatch'] = df_aircraft['title'].str.upper() != df_aircraft['text'].str.upper()

aircraft_names = list(df_aircraft['title'])

def get_infobox(wiki_html,title):
    """Grabs the infobox that appears on the top right side of each wiki 
    article"""
...
#for name in m_aircraft_names: # this was used to add the additional m 
# aircraft names that were missed during the initial scraping

# Main loop to scrape aircraft information from wikipedia
for name in aircraft_names[:10]: # remove [:10] slicing to scrape ALL aircraft
    i += 1
    print(i, name)
